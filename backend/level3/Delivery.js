class Delivery {
  ...
  getDeliveryDate(nbHolidays) {
    let shippingDateFormat = new Date(this.pack.shipping_date);
    let newDate = new Date(this.pack.shipping_date);
    let newDay =
      shippingDateFormat.getDate() +
      1 +
      this.carrier.delivery_promise +
      nbHolidays;
    return this.changeDateFormat(newDate.setDate(newDay)); // setDate takes care of edge cases
  }

  computeDistanceInDays() {
    let distance = this.countryDistance[this.pack.origin_country][
      this.pack.destination_country
    ];
  ...
  getDelivery() {
    ...
    if (sundayPresent) {
      ...
      if (
        this.carrier.saturday_deliveries === false &&
        saturdayAndSundayPresent
      ) {
        nbHolidays += 1;
      }
    } else if (
    ...
    nbHolidays += overseaDelay;

    this.delivery.package_id = this.pack.id;
    this.delivery.expected_delivery = this.getDeliveryDate(nbHolidays);
