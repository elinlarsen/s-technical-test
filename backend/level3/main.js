...
let deliveries= new getDeliveries(carriersInfo) //new Deliveries(carriersInfo).getDeliveries()
...
fs.writeFile('./data/output.json', jsonStringDeliveries, err => {
    if (err) {
        ...
        console.log("computed and expected deliveries are equal ? ", _.isEqual(deliveries, expectedDeliveries))
    }
})
